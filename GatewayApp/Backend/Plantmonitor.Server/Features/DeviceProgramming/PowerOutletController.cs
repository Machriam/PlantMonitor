...
public class PowerOutletController(DataContext context)
{
    ...
    public async Task SwitchOutlet([FromServices] IDeviceApiFactory apiFactory, string ip, long codeId)
    {
        var code = context.SwitchableOutletCodes.First(c => c.Id == codeId).Code;
        await apiFactory.SwitchOutletsClient(ip).SwitchoutletAsync(code);
    }
}
