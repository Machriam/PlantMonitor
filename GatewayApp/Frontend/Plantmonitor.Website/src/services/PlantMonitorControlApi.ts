...
export class ImageTakingClient extends PlantMonitorControlApiBase implements IImageTakingClient {
    ...
    getVideoStream(): Promise<FileResponse> {
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.transformResult(url_, _response, (_response: Response) => this.processGetVideoStream(_response));
        });
    }
    ...
    getVideoTest(): Promise<FileResponse> {
        let url_ = this.baseUrl + "/api/ImageTaking/videotest";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.transformResult(url_, _response, (_response: Response) => this.processGetVideoTest(_response));
        });
    }

    protected processGetVideoTest(response: Response): Promise<FileResponse> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200 || status === 206) {
            const contentDisposition = response.headers ? response.headers.get("content-disposition") : undefined;
