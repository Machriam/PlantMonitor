...
export interface ICustomTourClient {

    availableTours(ip?: string | undefined): Promise<CustomTourData[]>;

    downloadFile(ip?: string | undefined, fileName?: string | undefined): Promise<void>;
}

export class CustomTourClient extends GatewayAppApiBase implements ICustomTourClient {
    private http: { fetch(url: RequestInfo, init?: RequestInit): Promise<Response> };
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
    ...
    availableTours(ip?: string | undefined): Promise<CustomTourData[]> {
        let url_ = this.baseUrl + "/api/CustomTour/availabletours?";
        if (ip === null)
            throw new Error("The parameter 'ip' cannot be null.");
        else if (ip !== undefined)
            url_ += "ip=" + encodeURIComponent("" + ip) + "&";
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.transformResult(url_, _response, (_response: Response) => this.processAvailableTours(_response));
        });
    }

    protected processAvailableTours(response: Response): Promise<CustomTourData[]> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            ...
            if (Array.isArray(resultData200)) {
                result200 = [] as any;
                for (let item of resultData200)
                    result200!.push(CustomTourData.fromJS(item));
            }
            else {
                result200 = <any>null;
            }
        ...
        return Promise.resolve<CustomTourData[]>(null as any);
    }

    downloadFile(ip?: string | undefined, fileName?: string | undefined): Promise<void> {
        let url_ = this.baseUrl + "/api/CustomTour/downloadfromdevice?";
        if (ip === null)
            throw new Error("The parameter 'ip' cannot be null.");
        else if (ip !== undefined)
            url_ += "ip=" + encodeURIComponent("" + ip) + "&";
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.transformResult(url_, _response, (_response: Response) => this.processDownloadFile(_response));
        });
    }
...
export class CustomTourData implements ICustomTourData {
    deviceData!: AvailableCustomData;
    availableOnDevice!: boolean;
    stored!: boolean;

    constructor(data?: ICustomTourData) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    init(_data?: any) {
        if (_data) {
            this.deviceData = _data["DeviceData"] ? AvailableCustomData.fromJS(_data["DeviceData"]) : <any>undefined;
            this.availableOnDevice = _data["AvailableOnDevice"];
            this.stored = _data["Stored"];
        }
    }

    static fromJS(data: any): CustomTourData {
        data = typeof data === 'object' ? data : {};
        let result = new CustomTourData();
        result.init(data);
        return result;
    }
    ...
    toJSON(data?: any) {
        data = typeof data === 'object' ? data : {};
        data["DeviceData"] = this.deviceData ? this.deviceData.toJSON() : <any>undefined;
        data["AvailableOnDevice"] = this.availableOnDevice;
        data["Stored"] = this.stored;
        return data;
    }

    clone(): CustomTourData {
        const json = this.toJSON();
        let result = new CustomTourData();
        result.init(json);
        return result;
...
export interface ICustomTourData {
    deviceData: AvailableCustomData;
    availableOnDevice: boolean;
    stored: boolean;
}

export class AvailableCustomData implements IAvailableCustomData {
    sizeInMb!: number | undefined;
    fileName!: string | undefined;

    constructor(data?: IAvailableCustomData) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): AvailableCustomData {
        data = typeof data === 'object' ? data : {};
        let result = new AvailableCustomData();
        result.init(data);
        return result;
    }
...
export interface IAvailableCustomData {
    sizeInMb: number | undefined;
    fileName: string | undefined;
}
