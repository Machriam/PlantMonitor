...
1. Making it easy to monitor the growth process and plant specific behaviours like bending of the leaves during heatstress events in real time.
1. This is a all in one solution, this means you build the hardware and the software takes care of the calibration, image taking, processing of data and illustrating the results of the images.
1. This software can also monitor anything else, as the segmentation is highly customizable and the user decides, what to look at.
2. The Plantmonitor gives easy access to all raw data taken during an experiment.
3. This is a web based project and any device with a browser can be used to control the experiment.
...
- Those are the images taking during a single photo trip.
...
- With the "Save Photo Trip" button you can also store the whole trip. The images are stored in full size, therefor no preview is shown. However it displays the transmitted images after the trip, so that the users sees, when the transmission has finished.
...
- Global alignment of VIS and IR image. This feature is found in the Replay Pictures menu. You have to select a VIS image on one side and an IR image on the other side. Then you can align those images by clicking "Align images".
- For easier alignment it is recommend to overlay images taken during heat stress or cool down.  
...
- Previously defined alignments and plant positions are overlayed pixel perfectly
- 3 virtual images with the same dimensions and spacing are exported as png within a zip file. The zip archives contains the following files:
   1. VIS image 
   2. An rainbow IR image with a human-friendly color spectrum
...
- You can scroll in the chart to zoom in or out of datapoints.
- Summary data of plant growth can also be exported for use in other visualisation software. Press "Download Data" to get a zipped JSON export of the data. 
...
- In the "Virtual Photos" tab you can download all virtual images in a zip archive. Zipping all virtual images takes a while and this runs in the background. A 3 week long photo tour with a trip intervall of 4 minutes generates around 36 GB of virtual image data.
...
- Sometimes it is hard so see plants of interest in the whole virtual image. You can only display the selected plants in the plant summary via one click.
...
- This feature makes it possible to segment anything. You can change the hue range, to only segment blue, red oder yellow parts in the image. This means you can measure the size of anything, as long as it is visibly distinguishable from the background.
- The current version features also a particle filter.
...
- Configure the correct SSH user and password with "Update Password" to allow the gateway server to login into the devices
- Press the "Configure" button and after around 2-3 minutes the setup should be finished
- You should be able to use any Raspberry Pi with atleast the processing power of a Raspberry Pi 3 / Raspberry Pi Zero 2
...
- This is achieved via using tmux as a terminal multiplexer. The first thing the "Configure" button will do is to ensure, that tmux is installed
...
|Word|Definition|
|---|---|
|Photo tour|A photo tour is a collection of photo trips. A photo tour defines the intervall between each photo trip and which devices and sensors should participate in the creation of trips.|
...
|Movement plan|Contains the definition of all positions the motor should move to during a photo trip. At each location the motor stops and the configured cameras take on image of the location. The best IR and VIS image is taken after the IR camera has issued a FFC|
...
|Polygon|Plantmonitor must know, which plants exist and from which camera position and coordinates to take it from. This is where the user can define polygons or cuts on the image, which are then used for further processing.|
...
|Global Alignment|The IR and VIS camera have a distance from each other and a different field of view. The global alignment is a user supplied rough estimation to correct this. Correction means to overlay VIS and IR image as good as possible.|
|Fine Alignment|The global alignment might never be exact. That is why a fine alignment for each plant is needed. Each individual plant is overlayed in the VIS and IR image as good as possible.|
|Custom Tour|Is a hacked photo tour, which takes photos as fast as possible without reading sensors or moving automatically. The resulting zip can be imported as usual photo tour, where each combination of taken IR and VIS image creates a trip.|
